...
EnemyManager.prototype.Update = function(gametime) {
    ...
    if ( this.counter <= 0) {
        ...
        if (Math.random() < 0.5)
            this.planes.push(new Plane(this.planes_path[this.plane_type], 0, 50 * current_lane ,veloObj,1,points,this.sound_paths[this.plane_type]));
        else
            this.planes.push(new Plane(this.planes_path[this.plane_type],$("#canvas").width(), 50 * current_lane ,this.negateVelocity(veloObj),-1,points,this.sound_paths[this.plane_type]));        
        this.counter = this.time_delay;        
    }    
    else
    {
