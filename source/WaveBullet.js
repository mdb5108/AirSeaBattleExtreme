...
WaveBullet.prototype.Update = function(gameTime)
{
    ...
    var displacement = VectorAdd(VectorMultiply(this.speed*this.time, this.direction), VectorMultiply(this.waveMagnitude*Math.sin(this.time*this.waveFrequency), VectorPerpendicular(this.direction)));
    this.x = this.originalX+displacement.x;
    this.y = this.originalY+displacement.y;
    this.angle = this.originalAngle + Math.cos(this.time*this.waveFrequency);
